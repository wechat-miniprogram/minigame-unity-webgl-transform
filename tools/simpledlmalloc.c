...
#if LOCK_AT_FORK
static void pre_fork(void)         { ACQUIRE_LOCK(&(dl_gm)->mutex); }
static void post_fork_parent(void) { RELEASE_LOCK(&(dl_gm)->mutex); }
static void post_fork_child(void)  { INITIAL_LOCK(&(dl_gm)->mutex); }
#endif /* LOCK_AT_FORK */
...
static int init_mparams(void) {
    ...
    if (mparams.magic == 0) {
        ...
        dl_gm->mflags = mparams.default_mflags;
        (void)INITIAL_LOCK(&dl_gm->mutex);
#endif
#if LOCK_AT_FORK
...
((m == dl_gm)? dlmalloc(b) : mspace_malloc(m, b))
#define internal_free(m, mem)\
if (m == dl_gm) dlfree(mem); else mspace_free(m,mem);
#else /* MSPACES */
...
void* dlmalloc(size_t bytes) {
    ...
    if (!PREACTION(dl_gm)) {
        ...
        if (bytes <= MAX_SMALL_REQUEST) {
            ...
            idx = small_index(nb);
            smallbits = dl_gm->smallmap >> idx;
            
            if ((smallbits & 0x3U) != 0) { /* Remainderless fit to a smallbin. */
                mchunkptr b, p;
                idx += ~smallbits & 1;       /* Uses next bin if idx empty */
                b = smallbin_at(dl_gm, idx);
                p = b->fd;
                assert(chunksize(p) == small_index2size(idx));
                unlink_first_small_chunk(dl_gm, b, p, idx);
                set_inuse_and_pinuse(dl_gm, p, small_index2size(idx));
                mem = chunk2mem(p);
                check_malloced_chunk(dl_gm, mem, nb);
                goto postaction;
            }
            
            else if (nb > dl_gm->dvsize) {
                if (smallbits != 0) { /* Use chunk in next nonempty smallbin */
                    ...
                    binmap_t leastbit = least_bit(leftbits);
                    compute_bit2idx(leastbit, i);
                    b = smallbin_at(dl_gm, i);
                    p = b->fd;
                    assert(chunksize(p) == small_index2size(i));
                    unlink_first_small_chunk(dl_gm, b, p, i);
                    rsize = small_index2size(i) - nb;
                    /* Fit here cannot be remainderless if 4byte sizes */
                    if (SIZE_T_SIZE != 4 && rsize < MIN_CHUNK_SIZE)
                        set_inuse_and_pinuse(dl_gm, p, small_index2size(i));
                    else {
                        set_size_and_pinuse_of_inuse_chunk(dl_gm, p, nb);
                        r = chunk_plus_offset(p, nb);
                        set_size_and_pinuse_of_free_chunk(r, rsize);
                        replace_dv(dl_gm, r, rsize);
                    }
                    mem = chunk2mem(p);
                    check_malloced_chunk(dl_gm, mem, nb);
                    goto postaction;
                ...
                else if (dl_gm->treemap != 0 && (mem = tmalloc_small(dl_gm, nb)) != 0) {
                    check_malloced_chunk(dl_gm, mem, nb);
                    goto postaction;
                }
            }
            ...
            if (dl_gm->treemap != 0 && (mem = tmalloc_large(dl_gm, nb)) != 0) {
                check_malloced_chunk(dl_gm, mem, nb);
                goto postaction;
            }
        }
        ...
        if (nb <= dl_gm->dvsize) {
            size_t rsize = dl_gm->dvsize - nb;
            mchunkptr p = dl_gm->dv;
            if (rsize >= MIN_CHUNK_SIZE) { /* split dv */
                mchunkptr r = dl_gm->dv = chunk_plus_offset(p, nb);
                dl_gm->dvsize = rsize;
                set_size_and_pinuse_of_free_chunk(r, rsize);
                set_size_and_pinuse_of_inuse_chunk(dl_gm, p, nb);
            }
            else { /* exhaust dv */
                size_t dvs = dl_gm->dvsize;
                dl_gm->dvsize = 0;
                dl_gm->dv = 0;
                set_inuse_and_pinuse(dl_gm, p, dvs);
            }
            mem = chunk2mem(p);
            check_malloced_chunk(dl_gm, mem, nb);
            goto postaction;
        ...
        else if (nb < dl_gm->topsize) { /* Split top */
            size_t rsize = dl_gm->topsize -= nb;
            mchunkptr p = dl_gm->top;
            mchunkptr r = dl_gm->top = chunk_plus_offset(p, nb);
            r->head = rsize | PINUSE_BIT;
            set_size_and_pinuse_of_inuse_chunk(dl_gm, p, nb);
            mem = chunk2mem(p);
            check_top_chunk(dl_gm, dl_gm->top);
            check_malloced_chunk(dl_gm, mem, nb);
            goto postaction;
        }
        
        mem = sys_alloc(dl_gm, nb);
...
void* dlmemalign(size_t alignment, size_t bytes) {
    ...
    return internal_memalign(dl_gm, alignment, bytes);
}
    ...
    if (alignment == MALLOC_ALIGNMENT)
        ...
        if (r != 0 || d == 0 || (d & (d-SIZE_T_ONE)) != 0)
            ...
            if (alignment <  MIN_CHUNK_SIZE)
                alignment = MIN_CHUNK_SIZE;
            mem = internal_memalign(dl_gm, alignment, bytes);
        }
    }
    if (mem == 0)
        return ENOMEM;
...
void** dlindependent_calloc(size_t n_elements, size_t elem_size,
    ...
    return ialloc(dl_gm, n_elements, &sz, 3, chunks);
}

void** dlindependent_comalloc(size_t n_elements, size_t sizes[],
                              void* chunks[]) {
    return ialloc(dl_gm, n_elements, sizes, 0, chunks);
}

size_t dlbulk_free(void* array[], size_t nelem) {
    return internal_bulk_free(dl_gm, array, nelem);
}
...
void dlmalloc_inspect_all(void(*handler)(void *start,
    ...
    ensure_initialization();
    if (!PREACTION(dl_gm)) {
        internal_inspect_all(dl_gm, handler, arg);
        POSTACTION(dl_gm);
    }
}
#endif /* MALLOC_INSPECT_ALL */
...
size_t dlmalloc_footprint(void) {
    return dl_gm->footprint;
}

size_t dlmalloc_max_footprint(void) {
    return dl_gm->max_footprint;
}

size_t dlmalloc_footprint_limit(void) {
    size_t maf = dl_gm->footprint_limit;
...
size_t dlmalloc_set_footprint_limit(size_t bytes) {
    ...
    return dl_gm->footprint_limit = result;
}

#if !NO_MALLINFO
struct mallinfo dlmallinfo(void) {
    return internal_mallinfo(dl_gm);
}
#endif /* NO_MALLINFO */

#if !NO_MALLOC_STATS
void dlmalloc_stats() {
    internal_malloc_stats(dl_gm);
}
#endif /* NO_MALLOC_STATS */
