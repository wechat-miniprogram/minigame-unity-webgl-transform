...
public class AssetBundleLoad
{
    public static IEnumerator Init()
    {
...
#elif UNITY_EDITOR
        string[] luaAb = Directory.GetFiles(
        ...
        foreach (var name in luaAb)
        {
            ...
            {
                string bundname = luaFiles[i]
                    .Substring(0, luaFiles[i].LastIndexOf("/"))
                    .Replace("assets/temp/", "")
                    .Replace("/", "_");
...
#if UNITY_WEBGL && !UNITY_EDITOR
        WWW www2 = WWW.LoadFromCacheOrDownload(
...
#if UNITY_EDITOR
        var manifest = AssetBundle.LoadFromFile(
            Application.streamingAssetsPath + "/StreamingAssets"
        );
        Debug.Log("manifest:" + manifest);
#endif
