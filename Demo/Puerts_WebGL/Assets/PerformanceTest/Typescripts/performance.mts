...
CS.PerformanceHelper.JSNumber.text = 'Puerts Number Call:' + (Date.now() - start) + 'ms'
...
CS.PerformanceHelper.JSVector.text = 'Puerts Vector Call:' + (Date.now() - start2) + 'ms'

// const fibcache = [0, 1]
function fibonacci(level: number): number {
    if (level == 0) return 0;
    if (level == 1) return 1;
