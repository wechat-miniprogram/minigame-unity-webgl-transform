...
public class LinkXmlGen : ScriptableObject
{
    ...
    public static IEnumerable<CustomGenTask> GetTasks(LuaEnv lua_env, UserConfig user_cfg)
    {
        ...
        var assembly_infos = (
            from type in (user_cfg.ReflectionUse.Concat(user_cfg.LuaCallCSharp))
            group type by type.Assembly.GetName().Name into assembly_info
            select new { FullName = assembly_info.Key, Types = assembly_info.ToList() }
        ).ToList();
        ...
        yield return new CustomGenTask
        {
            ...
            Output = new StreamWriter(
                GeneratorConfig.common_path + "/link.xml",
                false,
                Encoding.UTF8
            )
        };
