...
public class ScriptBehaviourManager {
    ...
    public static void InvokeUpdate<T>(T mb) where T : MonoBehaviour 
    {
        Action _callback;
        var mbType = typeof(T);
        if (callbacks.TryGetValue(mbType, out _callback)) {
    ...
    private static Dictionary<Type, Action<MonoBehaviour>> starters = new Dictionary<Type, Action<MonoBehaviour>>();
    public static void InvokeStarter<T>(T mb, string ScriptName) where T : MonoBehaviour 
    {
        ...
        if (!starters.TryGetValue(mbType, out starter)) {
            ...
            {
                if (luaEnv == null) luaEnv = new XLua.LuaEnv();
            ...
            {
                if (jsEnv == null) {
                    var loader = new TSLoader(System.IO.Path.Combine(Application.dataPath, "../Puer-Project/out"));
