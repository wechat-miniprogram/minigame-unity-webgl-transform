...
namespace BuildReportTool
{
    ...
    public static class AssetDependencyGenerator
    {
        ...
        public static void Create(
            AssetDependencies data,
            BuildReportTool.BuildInfo.SceneInBuild[] scenes,
            bool debugLog = false
        )
        {
            var startingOpenSet = new Queue<string>();
        ...
        )
        {
            ...
            if (buildInfo.UsedAssets != null && buildInfo.UsedAssets.All != null)
            {
                ...
                {
                    if (
                        !string.IsNullOrEmpty(allUsedAssets[n].Name)
                        && allUsedAssets[n]
                            .Name.IndexOf("/Resources/", StringComparison.OrdinalIgnoreCase) > -1
                    )
                    {
        ...
        )
        {
            ...
            for (int n = idx - 1; n >= 0; --n)
            {
                if (list[n].IndentLevel == list[idx].IndentLevel - 1)
                {
                    idxOfPrevious = n;
                    return list[n]
                        .AssetPath.EndsWith(fileTypeInPrevious, StringComparison.OrdinalIgnoreCase);
                }
            }
        ...
        static bool IsInstanceOfFbxUsingMaterial(
            string fbxAssetPath,
        ...
        static bool AreInstancesOfFbxUsingMaterial(
            string fbxAssetPath,
            string materialAssetPath,
            Dictionary<string, DependencyEntry> assetDependencies,
            bool debugLog = false
        ...
        static bool Contains(
            List<AssetUserFlattened> list,
            AssetUserFlattened check,
            string assetParentOfCheck
        )
        {
            for (int n = list.Count - 1; n >= 0; --n)
            {
                ...
                if (
                    list[n].AssetPath.Equals(check.AssetPath, StringComparison.OrdinalIgnoreCase)
        ...
        )
        {
            ...
            while (openSet.Count > 0)
            {
                ...
                string[] foundDependencies = GetDependencies(
                    newAssetToInspect,
            ...
            foreach (var pair in assetDependencies)
            {
                ...
                {
                    AssetUserFlattened newEntry = new AssetUserFlattened(
                        assetUsers[n],
                        1
                ...
                while (openFlattenedSet.Count > 0)
                {
                    ...
                    if (usersFlattened.Count >= 2)
                    {
                        ...
                        if (userAsset.IndentLevel > 1)
                        {
                            ...
                            if (
                                debugLog
                                && pair.Key.Contains("Runtime-Only DLL/TextMeshPro.dll")
                            ...
                            for (int traceN = usersFlattened.Count - 2; traceN >= 0; --traceN)
                            {
                                if (usersFlattened[traceN].IndentLevel > currentIndentLevel)
                                {
                                    ...
                                    if (
                                        debugLog
                                        && pair.Key.Contains("Runtime-Only DLL/TextMeshPro.dll")
                                        && userAsset.AssetPath.Contains(
                                            "AbilityEntry Settings.asset"
                                        )
                                    )
                                    {
                                        Debug.LogFormat(
                                            "Skipped element {0} since its indent level is: {1} (looking for {2})",
                                            traceN,
                                ...
                                {
                                    Debug.LogFormat(
                                        "Now at element {0}: {1} at indent level {2}. Checking if same as {3}",
                                        traceN,
                                ...
                                if (usersFlattened[traceN].IndentLevel <= 1)
                                {
                                    ...
                                    {
                                        Debug.LogFormat(
                                            "Element {0} has indent level of {1} so aborting the check",
                                            traceN,
                                            usersFlattened[traceN].IndentLevel.ToString()
                                        );
                                    }
                        ...
                        int idxOfPrevious;
                        if (
                            !noNeedToAddUsersOfUser
                            && IsFileTypeBeforeAnother(
                                usersFlattened,
                        ...
                        )
                        {
                            ...
                            var isMaterialReallyBeingUsed = AreInstancesOfFbxUsingMaterial(
                                fbxUsingTheMaterialAsDefault,
                                materialAssignedAsDefault,
                                assetDependencies,
                                debugLog
                    ...
                    if (assetDependencies.ContainsKey(userAsset.AssetPath))
                    {
                        ...
                        for (int n = 0, len = usersOfUser.Count; n < len; ++n)
                        {
                            ...
                            if (userAssetIsFbxUsingMaterial)
                            {
                                ...
                                if (
                                    usersOfUser[n]
                                        .EndsWith(".unity", StringComparison.OrdinalIgnoreCase)
                                )
                                {
