...
namespace BuildReportTool
{
    ...
    public class AssetDependencies
    {
        ...
        Dictionary<string, DependencyEntry> _assetDependencies =
            new Dictionary<string, DependencyEntry>();
        ...
        )
        {
            ...
            {
                if (
                    usersFlattened[n].AssetPath.IsSceneFile()
                ...
                )
                {
                    ...
                    for (
                        int alreadyN = 0, alreadyLen = availableExistingIdx;
                        alreadyN < alreadyLen;
                        ++alreadyN
                    )
                    {
                        if (
                            destination[alreadyN]
                                .text.Equals(assetFilename, StringComparison.OrdinalIgnoreCase)
                        )
                        {
                            alreadyInList = true;
                            break;
                    ...
                    else
                    {
                        ...
                        destination[availableExistingIdx].image = AssetDatabase.GetCachedIcon(
                            usersFlattened[n].AssetPath
                        );
