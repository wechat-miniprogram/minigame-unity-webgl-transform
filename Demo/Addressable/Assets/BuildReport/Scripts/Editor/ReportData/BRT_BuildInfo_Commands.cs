...
namespace BuildReportTool
{
    public partial class BuildInfo
    {
        ...
        public bool HasContents
        {
            get
            {
                ...
                return !string.IsNullOrEmpty(ProjectName)
                    && (BuildSizes != null && BuildSizes.Length > 0);
            }
        }

        public bool IsUnityVersionAtLeast(int majorAtLeast, int minorAtLeast, int patchAtLeast)
        {
            return DldUtil.UnityVersion.IsUnityVersionAtLeast(
                UnityVersion,
                majorAtLeast,
                minorAtLeast,
                patchAtLeast
        ...
        public string SuitableTitle
        {
            ...
            {
                if (
                    UnityBuildSettings != null
                    && UnityBuildSettings.HasValues
        ...
        {
            return BuildReportTool.Util.GetBuildInfoDefaultFilename(
                ProjectName,
                BuildType,
                TimeGot
            );
        ...
        {
            return BuildReportTool.Util.GetAssetDependenciesDefaultFilename(
                ProjectName,
                BuildType,
                TimeGot
            );
        ...
        public bool HasOldSizeValues
        {
            get
            {
                return string.IsNullOrEmpty(UnusedTotalSize)
        ...
        void CalculateUsedAssetsDerivedSizes()
        {
            if (UsedAssets != null)
            {
                ...
                {
                    UsedAssets.All[n].DerivedSize = BuildReportTool.Util.GetApproxSizeFromString(
                        UsedAssets.All[n].Size
                    );
                }
            }
        }
        ...
        public void FixReport()
        {
            ...
            if (!DldUtil.UnityVersion.IsUnityVersionAtLeast(5, 2, 1))
            {
                ...
                for (int n = 0; n < UsedAssets.All.Length; ++n)
                {
                    ...
                    {
                        UsedAssets.All[n].ImportedSizeBytes = BRT_LibCacheUtil.GetImportedFileSize(
                            UsedAssets.All[n].Name
                        );
                        UsedAssets.All[n].ImportedSize = BuildReportTool.Util.GetBytesReadable(
                            UsedAssets.All[n].ImportedSizeBytes
                        );
                ...
                var resourcesTextureSizeSum = GetSizeSumForUsedAssets(
                    "/Resources/",
                    BuildReportTool.Util.IsTextureFile
                );
                AddToSize("Textures", resourcesTextureSizeSum);

                var resourcesMeshSizeSum = GetSizeSumForUsedAssets(
                    "/Resources/",
                    BuildReportTool.Util.IsMeshFile
                );
                AddToSize("Meshes", resourcesMeshSizeSum);

                var resourcesSoundsSizeSum = GetSizeSumForUsedAssets(
                    "/Resources/",
                    BuildReportTool.Util.IsSoundFile
                );
                AddToSize("Sounds", resourcesSoundsSizeSum);

                var resourcesAnimationsSizeSum = GetSizeSumForUsedAssets(
                    "/Resources/",
                    BuildReportTool.Util.IsAnimationFile
                );
        ...
        void RecalculatePercentages()
        {
            ...
            for (int n = 0, len = BuildSizes.Length; n < len; ++n)
            {
                BuildSizes[n].Percentage = System.Math.Round(
                    (BuildSizes[n].UsableSize / totalSize) * 100,
                    2,
        ...
        long GetSizeSumForUsedAssets(
            string assetFolderName,
            System.Func<string, bool> fileTypePredicate
        )
        {
        ...
        void AddToSize(string buildSizeName, long sizeToAdd)
        {
            ...
            BuildReportTool.SizePart buildSize = BuildSizes.FirstOrDefault(part =>
                part.Name == buildSizeName
            );
