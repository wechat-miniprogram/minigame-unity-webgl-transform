...
public class APIAssetPostprocessor : AssetPostprocessor
{
        ...
        foreach (var assetPath in addedAssets)
        {
            ...
            switch (Path.GetExtension(assetPath))
            {
                case ".asset":
                {
                    ...
                    else if (entrySO != null && entrySO is DetailsEntrySO detailsEntrySO)
                    {
                        UpdateCategorySO(GetCategorySOFromParentPath(assetPath));
                        UpdateDetailsEntrySO(detailsEntrySO);
                    }
                ...
                case ".cs":
                {
                    ...
                    if (entrySO is DetailsEntrySO detailsEntrySO)
                        UpdateDetailsEntrySO(detailsEntrySO);
    ...
    public static void UpdateAPISO(APISO apiSO)
    {
        ...
        foreach (var subdirectory in subdirectories)
        {
            ...
            foreach (var file in allFiles)
            {
                ...
                if (categorySO != null)
                {
                    if (categorySO.isNative)
                    {
                        apiSO.nativeAbilityList.Add(categorySO);
                    }
                    else
                    {
    ...
    public static void UpdateDetailsEntrySO(DetailsEntrySO entrySO)
    {
        if (entrySO == null)
            return;
        ...
        foreach (string scriptFile in scriptFiles)
        {
            ...
            if (
                scriptType != null
                && scriptType.IsSubclassOf(typeof(Details))
