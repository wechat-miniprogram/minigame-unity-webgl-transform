...
public class ReadAndWrite : Details
{
    ...
    private Action<WriteSuccessCallbackResult> onWriteSuccess = (res) =>
    {
        UpdateFileContent();
        WX.ShowModal(
            new ShowModalOption() { content = "Write Success, Result: " + JsonMapper.ToJson(res) }
        );
    };
    private Action<FileError> onWriteFail = (res) =>
    {
        WX.ShowModal(
            new ShowModalOption() { content = "Write Fail, Result: " + JsonMapper.ToJson(res) }
        );
    };
    ...
    {
        WX.ShowModal(new ShowModalOption() { content = "ReadSync接口暂无法正常使用" });
        return;
        var readResult = _fileSystemManager.ReadSync(
            new ReadSyncOption()
            {
                arrayBuffer = _buffer,
                fd = _fd,
        ...
        // 更新读取结果
        UpdateReadResult();
        WX.ShowToast(new ShowToastOption() { title = "ReadSync Success" });
    ...
    )
    {
        ...
        if (dataType == "string")
        {
            writeResult = _fileSystemManager.WriteSync(
                new WriteSyncStringOption()
                {
                    data = _stringData,
                    fd = _fd,
    ...
    )
    {
        if (dataType == "string")
        {
            _fileSystemManager.Write(
                new WriteStringOption()
                {
                    data = _stringData,
                    fd = _fd,
                    encoding = encoding == "null" ? null : encoding,
    ...
    private void UpdateReadResult()
    {
        ...
        GameManager.Instance.detailsController.ChangeResultContent(
            0,
            Encoding.UTF8.GetString(_buffer)
        );
    }
