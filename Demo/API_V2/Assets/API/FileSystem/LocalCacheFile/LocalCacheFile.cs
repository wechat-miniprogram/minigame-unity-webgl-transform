...
public class LocalCacheFile : Details
{
    ...
    protected override void TestAPI(string[] args)
    {
        ...
        WX.ShowToast(new ShowToastOption() { title = "刷新缓存文件列表成功" });
    }

    // 生成缓存文件
    private void GenerateCacheFile()
    {
        WX.ShowLoading(new ShowLoadingOption() { title = "下载临时文件中", mask = true });
        ...
        WX.DownloadFile(
            new DownloadFileOption()
            {
                ...
                success = (res) =>
                {
                    ...
                    WX.ShowLoading(new ShowLoadingOption() { title = "缓存文件中", mask = true });
                    ...
                    _fileSystemManager.SaveFile(
                        new SaveFileOption()
                        {
                            ...
                            success = (res2) =>
                            {
                                WX.HideLoading(new HideLoadingOption());
                                RefreshCacheFileList();
                                WX.ShowToast(new ShowToastOption() { title = "生成缓存文件成功" });
                            },
                            fail = (res2) =>
                            {
                                WX.HideLoading(new HideLoadingOption());
                                WX.ShowModal(
                                    ...
                                    {
                                        content =
                                            "SaveFile Fail, Result: " + JsonMapper.ToJson(res2)
                                    }
                                );
                            }
                        }
                        ...
                        _fileSystemManager.RemoveSavedFile(
                            new RemoveSavedFileOption()
                            {
                                ...
                                success = (res2) =>
                                {
                                    ...
                                    {
                                        RefreshCacheFileList();
                                        WX.ShowToast(new ShowToastOption() { title = "清空缓存文件成功" });
                                    }
                                },
                                fail = (res2) =>
                                {
                                    WX.ShowModal(
                                        ...
                                        {
                                            content =
                                                "RemoveSavedFile Fail, Result: "
                                                + JsonMapper.ToJson(res2)
                                        }
                                    );
                                }
                            }
        ...
        _fileSystemManager.GetSavedFileList(
            new GetSavedFileListOption()
            {
                success = (res) =>
                {
                    ...
                    {
                        // 如果没有缓存文件，提示用户
                        WX.ShowModal(new ShowModalOption() { content = "请先生成缓存文件！" });
                    }
                    else
                    {
                        // 获取第一个缓存文件的文件信息
                        _fileSystemManager.GetFileInfo(
                            new GetFileInfoOption()
                            {
