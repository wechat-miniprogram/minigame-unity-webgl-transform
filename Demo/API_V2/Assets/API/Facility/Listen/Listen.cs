...
public class Listen : Details
{
    ...
    private readonly Action<OnDeviceOrientationChangeListenerResult> _onDeviceOrientationChange = (
        res
    ) =>
    {
    ...
    public void Accelerometer()
    {
        ...
        GameManager.Instance.detailsController.ChangeInitialButtonText(
            _isListeningAccelerometer ? "取消监听加速度" : "开始监听加速度"
        );
    }
    ...
    public void Compass()
    {
        ...
        GameManager.Instance.detailsController.ChangeExtraButtonText(
            0,
            _isListeningCompass ? "取消监听罗盘" : "开始监听罗盘"
        );
    }
    ...
    public void DeviceMotion()
    {
        if (!_isListeningDeviceMotion)
        {
            WX.StartDeviceMotionListening(
                new StartDeviceMotionListeningOption
                {
                    interval = "normal",
                    success = (res) =>
                    {
        ...
        else
        {
            WX.StopDeviceMotionListening(
                new StopDeviceMotionListeningOption
                {
                    success = (res) =>
                    {
                        WX.ShowToast(
        ...
        GameManager.Instance.detailsController.ChangeExtraButtonText(
            1,
            _isListeningDeviceMotion ? "取消监听设备方向" : "开始监听设备方向"
        );
    }
    ...
    public void Gyroscope()
    {
        ...
        GameManager.Instance.detailsController.ChangeExtraButtonText(
            3,
            _isListeningGyroscope ? "取消监听陀螺仪" : "开始监听陀螺仪"
        );
    }
